/* =========================================================
   SCORE STORE — main.js (Vercel fused / corrected)
   - Vercel only: /api/catalog, /api/promos, /api/site_settings,
     /api/quote_shipping, /api/create_checkout, /api/chat
   - Keeps footer/contact/promo dynamic
   - Robust catalog image handling
   - Splash visible long enough to be perceived
   - Compatible with current index.html and legal.html
========================================================= */
(() => {
  "use strict";
  ...
  const safeUrl = (u) => {
    ...
    if (typeof u === "object") {
      return safeUrl(
        u.url || u.src || u.image_url || u.image || u.path || u.href || u.file || u.asset || ""
      );
    }
  ...
  const getProductSectionUi = (p) =>
    p?.uiSection || normalizeSectionToUi(p?.sectionId || p?.section_id || p?.section || p?.categoryId || "");
  ...
  const normalizeProduct = (row) => {
    const sectionRaw = String(row?.sectionId || row?.section_id || row?.categoryId || row?.section || "").trim();
    ...
    return {
      ...
      sub_section: String(row?.sub_section || row?.collection || "").trim(),
      image_url: images[0] || getProductImage(row),
      img: images[0] || getProductImage(row),
      images,
      sizes: getProductSizes(row),
  ...
  const els = {
    ...
    quoteBtn: $("#shipQuoteBtn") || $("#quoteBtn"),
    shipQuoteBtn: $("#shipQuoteBtn") || $("#quoteBtn"),
  ...
  const anyLayerOpen = () =>
    [els.sideMenu, els.cartDrawer, els.assistantModal, els.productModal, els.sizeGuideModal].some(
      (el) => el && !el.hidden
    );
  ...
  const setCheckoutLoading = (on) => {
    ...
    const submit = els.checkoutSubmitBtn || els.checkoutBtn;
    if (submit) submit.disabled = !!on || !!siteSettings.maintenance_mode || !cart.length;
  };
  ...
  const applyFooterAndPromo = () => {
    ...
    const submit = els.checkoutSubmitBtn || els.checkoutBtn;
    if (submit) {
      submit.disabled = !!siteSettings.maintenance_mode || !cart.length || loadingCheckout;
    }
  };
  ...
  const updateStatus = (count) => {
    ...
    els.statusRow.innerHTML = `<span class="status">${n} producto${n === 1 ? "" : "s"} encontrado${n === 1 ? "" : "s"}</span>`;
  };
  ...
  const productCardHTML = (p) => {
    ...
    return `
      <article class="product-card card glass-panel hover-fx" data-sku="${sku}">
        <div class="product-card__media card__media">
          <div class="product-card__track card__track" data-track>
            ${
              imgs.length
                ? imgs
                    .map((src) => `<img src="${escapeHtml(src)}" alt="${title}" loading="lazy" decoding="async">`)
                    .join("")
                : first
        ...
        <div class="product-card__body card__body">
          <div class="product-card__top card__top">${badge}</div>
          <h4 class="product-card__title card__title">${title}</h4>
          <p class="product-card__sku">${escapeHtml(String(p?.sku || ""))}</p>
          <div class="product-card__bottom card__row">
            <div class="price">${price}</div>
            <button class="btn btn--primary btn--tiny" type="button" data-open-product="${sku}">Ver detalle →</button>
          </div>
          ${count > 1 ? `<div class="product-card__more">+${count - 1} imagen${count - 1 === 1 ? "" : "es"}</div>` : ""}
        </div>
      </article>
    `;
  };
  ...
  const renderProducts = () => {
    ...
    $$(".product-card", els.productGrid).forEach((cardEl) => {
  ...
  const applyPromoCode = async (code) => {
    ...
    try {
      const res = await fetch(`${APP_ENDPOINTS.promos}?code=${encodeURIComponent(code)}`, {
        headers: { "cache-control": "no-store" },
      });
      if (res.ok) {
        const j = await res.json().catch(() => null);
  ...
  const quoteShipping = async () => {
    const postal = String(
      els.checkoutPostal?.value ||
      els.shipPostal?.value ||
      els.shipPostal?.value ||
      ""
    ).trim();
          ...
          ${
            imgs.length
              ? imgs
                  .map(
                    (src) =>
                      `<img src="${escapeHtml(src)}" width="400" height="500" loading="lazy" alt="${escapeHtml(
                        getProductName(p)
                      )}">`
                  )
                  .join("")
              : `<img src="${escapeHtml(getProductImage(p))}" width="400" height="500" loading="lazy" alt="${escapeHtml(
                  getProductName(p)
                )}">`
          }
        </div>
  ...
  const submitCheckout = async (ev) => {
    ...
    const promo_code = String(els.promoCode?.value || "").trim();
    ...
    const payload = {
      ...
      promo: activePromo,
      promo_code: promo_code || activePromo?.code || activePromo?.promo_code || "",
      payment_method,
      items: cart,
      totals: {
  ...
  const bindEvents = () => {
    ...
    els.shipModePickup?.addEventListener("change", () => {
      if (els.shipModePickup.checked) {
        shipMode = "pickup";
        try {
          localStorage.setItem(STORAGE_KEYS.shipMode, shipMode);
        } catch {}
        syncShipUI();
      }
    });
    ...
    els.quoteBtn?.addEventListener("click", quoteFromCurrentPostal);
    ...
    els.promoCode?.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        applyPromoCode(String(els.promoCode?.value || "").trim());
      }
    });
    ...
    els.scrollLeftBtn?.addEventListener("click", () => {
      els.productGrid?.scrollBy({ left: -700, behavior: "smooth" });
    });
    els.scrollRightBtn?.addEventListener("click", () => {
      els.productGrid?.scrollBy({ left: 700, behavior: "smooth" });
    });

    els.cookieAccept?.addEventListener("click", () => setCookieConsent("accept"));
    els.cookieReject?.addEventListener("click", () => setCookieConsent("reject"));
  };
  ...
  const normalizeCatalogPayload = (payload) => {
    ...
    const categoriesSource =
      ...
      Array.isArray(data.data?.sections) ? data.data.sections :
      [];
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      categories = normalized.categories.map(normalizeCategory);
      products = normalized.products.map(normalizeProduct).filter((p) => p.sku || p.title || p.name);
  ...
  const boot = async () => {
    ...
    const splashFailSafe = setTimeout(() => hideSplash(true), 12000);
