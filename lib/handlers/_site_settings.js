// lib/handlers/_site_settings.js
// (antes: api/_site_settings.js — movido a lib/handlers/)
"use strict";
...
module.exports = async (req, res) => {
  ...
  if (req.method === "PATCH" || req.method === "POST") {
    ...
    const orgId =
      ...
      (await resolveScoreOrgId(sb));

    // FIX: solo un campo para la PK. organization_id es la columna canónica.
    // Se elimina la duplicación de org_id / organization_id en el onConflict.
    const payload = {
      organization_id:   orgId,            // ← PK real
    ...
    const { error } = await sb
      .from("site_settings")
      .upsert(payload, { onConflict: "organization_id" }); // FIX: solo una columna
